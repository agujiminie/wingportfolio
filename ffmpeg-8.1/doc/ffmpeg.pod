...
=item B<-force_key_frames[:>I<stream_specifier>B<] scd_metadata (>I<output,per-stream>B<)>
...
If the argument is C<scd_metadata>, ffmpeg will force a key frame if
the current frame contains a metadata entry with the key C<lavfi.scd.time>.
The metadata can be added by filters like C<scdet> and C<scdet_vulkan>.
Avoid inserting filters that duplicate frames after C<scdet>, as this can
