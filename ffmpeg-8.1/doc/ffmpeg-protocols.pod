...
the B<multiple_requests> option, as well as setting
B<short_seek_size> to the same value or higher. Doing so allows FFmpeg
to reuse a single HTTP connection wherever possible.


=item B<initial_request_size>

Limit the size of initial requests. Similar to C<request_size>, but only
...
Note that if enabling this option, it's strongly recommended to also enable
the B<multiple_requests> option, as well as setting
B<short_seek_size> to the same value or higher.
...
I<key>=I<val>. Standard percent-encoding (and using the plus sign for
...
=item B<localport, local_rtpport, localrtpport=>I<n>
...
=item B<local_rtcpport, localrtcpport=>I<n>B<'>
...
=item B<tls_verify, verify=>I<1|0>
...
=item B<cert_pem=>I<string>

Certificate PEM string


=item B<key_pem=>I<string>
...
=item B<stream Corresponds to SOCK_STREAM (reliable stream-oriented)>


=item B<datagram Corresponds to SOCK_DGRAM (unreliable packet-oriented)>


=item B<seqpacket Corresponds to SOCK_SEQPACKET (reliable packet-oriented)>
...
C<max_packet_size>. Ignored for SOCK_STREAM. Default is C<0>.
