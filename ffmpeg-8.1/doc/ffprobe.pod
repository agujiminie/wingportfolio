...
selected using B<-data_dump_format>). Coupled with
...
=item B<-data_dump_format> I<format>

Select a format used for the data dumps enabled with the B<-show_data>
option. The default is C<xxd> which is a hexdump format compatible with the
well-known B<xxd> program. C<base64> is also supported.
...
=item B<-c:>I<media_specifier>B< >I<codec_name>
...
C<v> (video), C<s> (subtitle), and C<d> (data).
