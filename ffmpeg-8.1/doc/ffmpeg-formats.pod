...
=item B<cenc_decryption_keys>

Dictionary of 16-byte key ID =E<gt> 16-byte key, both in hex, to decrypt files encrypted using ISO Common Encryption
...
By default, for backward compatibility, the MCC demuxer extracts just the EIA-608 and CEA-708 closed captions and returns a C<EIA_608> stream, ignoring all other VANC data.
You can change it to return all VANC data in a C<SMPTE_436M_ANC> data stream by setting B<-eia608_extract 0>
...
This retains all VANC data and inserts it into the output MXF file as a C<SMPTE_436M_ANC> data stream.
...
For multiplane formats, B<stride> is a list of line size for each
plane.
...
Read a rawvideo with B<ffplay>, assuming a pixel format of C<yuv420p>,
a video size of C<1080x1920>, has 8 bytes of padding in each line of Y plane,
...
specify how many times to play the content, C<0> causes an infinite
...
=item B<override_time_code_rate>

Override the C<Time Code Rate> value in the output. Defaults to trying to deduce from the stream's C<time_base>, which often doesn't work.

=item B<use_u_alias>

Use the C<U> alias for the byte sequence C<E1h 00h 00h 00h>.
Disabled by default because some F<.mcc> files disagree on whether it has 2 or 3 zero bytes.

=item B<mcc_version>

The MCC file format version. Must be either 1 or 2, defaults to 2.

=item B<creation_program>
...
Extract a MXF C<SMPTE_436M_ANC> stream from a MXF file and write it to a MCC file at 30 fps.
...
Extract EIA-608/CTA-708 closed captions from a F<.mp4> file and write them to a MCC file at 29.97 fps.
...
an input file with B<ffmpeg> to WebRTC:
...
=item B<rtp_history> I<integer>

Set the number of RTP history items to store.
Default value is 512.


=item B<authorization> I<string>

The optional Bearer token for WHIP Authorization.


=item B<cert_file> I<string>

The optional certificate file path for DTLS.


=item B<key_file> I<string>
